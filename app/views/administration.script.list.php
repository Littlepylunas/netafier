...
$widget = (new CWidget())
	...
	->setControls((new CTag('nav', true,
		(new CList())
			->addItem(new CRedirectButton(_('Create script'), 'netafier.php?action=script.edit'))
		))
			->setAttribute('aria-label', _('Content controls'))
	)
	->addItem((new CFilter((new CUrl('netafier.php'))->setArgument('action', 'script.list')))
...
foreach ($data['scripts'] as $script) {
	...
	$scriptsTable->addRow([
		...
		(new CCol(
			new CLink($script['name'], 'netafier.php?action=script.edit&scriptid='.$script['scriptid'])
