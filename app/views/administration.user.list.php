...
$widget = (new CWidget())
	...
	->setControls((new CList([
		(new CForm('get'))
			...
			(new CTag('nav', true,
				(new CList())
					->addItem(new CRedirectButton(_('Create user'), 'netafier.php?action=user.edit'))
				))->setAttribute('aria-label', _('Content controls'))
		]))
	)
	->addItem((new CFilter((new CUrl('netafier.php'))->setArgument('action', 'user.list')))
		...
		->addFilterTab(_('Filter'), [
			...
			(new CFormList())->addRow(_('User type'),
				(new CRadioButtonList('filter_type', (int) $data['filter']['type']))
					->addValue(_('Any'), -1)
					->addValue(user_type2str(USER_TYPE_NETAFIER_USER), USER_TYPE_NETAFIER_USER)
					->addValue(user_type2str(USER_TYPE_NETAFIER_ADMIN), USER_TYPE_NETAFIER_ADMIN)
...
foreach ($data['users'] as $user) {
	...
	$blocked = ($user['attempt_failed'] >= ZBX_LOGIN_ATTEMPTS)
		? (new CLink(_('Blocked'), 'netafier.php?action=user.unblock&userids[]='.$userid))
	...
	$alias = new CLink($user['alias'], (new CUrl('netafier.php'))
