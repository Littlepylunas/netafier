...
$widget = (new CWidget())
	...
	->setControls((new CTag('nav', true,
		(new CList())
			->addItem(new CRedirectButton(_('Create media type'), 'netafier.php?action=mediatype.edit'))
			->addItem(new CRedirectButton(_('Import'), 'conf.import.php?rules_preset=mediatype'))
		))
			->setAttribute('aria-label', _('Content controls'))
	)
	->addItem((new CFilter((new CUrl('netafier.php'))->setArgument('action', 'mediatype.list')))
	...
	new CActionButtonList('action', 'mediatypeids', [
		...
		'mediatype.export' => ['name' => _('Export'), 'redirect' =>
			(new CUrl('netafier.php'))
				...
				->setArgument('backurl', (new CUrl('netafier.php'))
					->setArgument('action', 'mediatype.list')
