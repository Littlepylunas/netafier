...
$widget = (new CWidget())
	...
	->setControls((new CTag('nav', true,
		(new CList())
			->addItem(new CRedirectButton(_('Create proxy'), 'netafier.php?action=proxy.edit'))
		))
			->setAttribute('aria-label', _('Content controls'))
	)
	->addItem((new CFilter((new CUrl('netafier.php'))->setArgument('action', 'proxy.list')))
...
foreach ($data['proxies'] as $proxy) {
	...
	$name = new CLink($proxy['host'], 'netafier.php?action=proxy.edit&proxyid='.$proxy['proxyid']);
