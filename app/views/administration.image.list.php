...
$page_url = (new CUrl('netafier.php'))->setArgument('action', 'image.list');
$widget = (new CWidget())
	->setTitle(_('Images'))
			...
			->addItem((new CList())
				...
				->addItem(
					(new CButton(null, ($data['imagetype'] == IMAGE_TYPE_ICON)
						? _('Create icon')
						: _('Create background')
					))->onClick(sprintf('javascript: document.location="%s";', (new CUrl('netafier.php'))
...
if (!$data['images']) {
	...
	$edit_url = (new Curl('netafier.php'))->setArgument('action', 'image.edit');
