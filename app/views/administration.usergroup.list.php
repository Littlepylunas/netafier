	...
	->setControls((new CTag('nav', true,
		(new CList())
			->addItem(new CRedirectButton(_('Create user group'),
				(new CUrl('netafier.php'))->setArgument('action', 'usergroup.edit'))
			)
		))->setAttribute('aria-label', _('Content controls'))
	)
	->addItem((new CFilter((new CUrl('netafier.php'))->setArgument('action', 'usergroup.list')))
...
foreach ($this->data['usergroups'] as $usergroup) {
	$debug_mode = ($usergroup['debug_mode'] == GROUP_DEBUG_MODE_ENABLED)
		? (new CLink(_('Enabled'), (new CUrl('netafier.php'))
			...
			->addSID()
		: (new CLink(_('Disabled'), (new CUrl('netafier.php'))
			->setArgument('action', 'usergroup.massupdate')
	...
	foreach ($usergroup['users'] as $user) {
		...
		$users[] = (new CLink(getUserFullname($user), (new CUrl('netafier.php'))
	...
	$name = new CLink($usergroup['name'], (new CUrl('netafier.php'))
	...
	$table->addRow([
		...
		[new CLink(_('Users'), (new CUrl('netafier.php'))
...
$form->addItem([
	...
	new CActionButtonList('action', 'usrgrpids', [
		['name' => _('Enable'), 'confirm' => _('Enable selected groups?'),
			'redirect' => (new CUrl('netafier.php'))
				->setArgument('action', 'usergroup.massupdate')
				->setArgument('users_status', GROUP_STATUS_ENABLED)
				->getUrl()
		],
