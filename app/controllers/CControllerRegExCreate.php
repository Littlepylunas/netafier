...
class CControllerRegExCreate extends CController {
		...
		if (!$ret) {
			switch ($this->getValidationError()) {
				case self::VALIDATION_ERROR:
					$url = (new CUrl('netafier.php'))->setArgument('action', 'regex.edit');
	...
	protected function doAction() {
		...
		if ($result) {
			$response = new CControllerResponseRedirect((new CUrl('netafier.php'))->setArgument('action', 'regex.list'));
			$response->setMessageOk(_('Regular expression added'));
		}
		else {
			$response = new CControllerResponseRedirect((new CUrl('netafier.php'))->setArgument('action', 'regex.edit'));
			$response->setFormData($this->getInputAll());
			$response->setMessageError(_('Cannot add regular expression'));
		}
