...
class CHost extends CHostGeneral {
	...
	public function create($hosts) {
		...
		foreach ($hosts as $host) {
			...
			// NGOCVB_START_CHANGE
			// Create screen default.
			if (strpos(strtolower($host['host']), 'switch') !== false) {
				$screenToAdd = [];
				$hostname = $host['host'].'-screen';
				$screenToAdd[] = [
					'name' => $hostname,
					'hsize' => 2,
					'vsize' => 4,
				...
				$screenids = DB::insert('screens', $screenToAdd);
				...
				$screenItemToAdd1[] = [
					...
					'resourcetype' => 16, 
					'resourceid' => $hostid, 
					'width' => 500, 'height' => 100, 'x' => 0, 'y' => 2, 
				 	'colspan' => 2, 'rowspan' => 1, 'elements' => 5, 'valign' => 0, 
					'halign' => 0, 'style' => 0, 'url' => '', 'dynamic' => 0, 
					'sort_triggers' => 0, 'application' => '', 'max_columns' => 3
				];
				DB::insert('screens_items', $screenItemToAdd1);

				// Processor: Memory utilization. vm.memory.util[vm.memory.util.1]
				$screenItemToAdd2[] = [
					'screenid' => $screenid, 
					'resourcetype' => 1, 
					'resourceid' => $screenid * $screenid + 1, 
					'width' => 500, 'height' => 100, 'x' => 1, 'y' => 0, 
				 	'colspan' => 1, 'rowspan' => 1, 'elements' => 0, 'valign' => 0, 
					'halign' => 0, 'style' => 0, 'url' => '', 'dynamic' => 1, 
					'sort_triggers' => 0, 'application' => '', 'max_columns' => 3
				];
				DB::insert('screens_items', $screenItemToAdd2);
				// DBExecute("UPDATE screens_items set resourceid = (SELECT itemid from items 
				// 	WHERE hostid = '$hostid' and key_ = 'vm.memory.util[vm.memory.util.1]') 
				// WHERE resourceid = 999999999");

				// SW#1, Sensor#1, GREEN : Temperature. sensor.temp.value[ciscoEnvMonTemperatureValue.1008]
				$screenItemToAdd3[] = [
					'screenid' => $screenid, 
					'resourcetype' => 1, 
					'resourceid' => $screenid * $screenid + 2, 
					'width' => 500, 'height' => 100, 'x' => 1, 'y' => 1, 
					'colspan' => 1, 'rowspan' => 1, 'elements' => 0, 'valign' => 0, 
					'halign' => 0, 'style' => 0, 'url' => '', 'dynamic' => 0, 
					'sort_triggers' => 0, 'application' => '', 'max_columns' => 3
				];
				DB::insert('screens_items', $screenItemToAdd3);

				// ICMP response time. icmppingsec
				$screenItemToAdd4[] = [
					'screenid' => $screenid, 
					'resourcetype' => 1, 
					'resourceid' => $screenid * $screenid + 3, 
					'width' => 500, 'height' => 100, 'x' => 0, 'y' => 1, 
					'colspan' => 1, 'rowspan' => 1, 'elements' => 0, 'valign' => 0, 
					'halign' => 0, 'style' => 0, 'url' => '', 'dynamic' => 1, 
					'sort_triggers' => 0, 'application' => '', 'max_columns' => 3
				];
				DB::insert('screens_items', $screenItemToAdd4);

				#1: CPU utilization. system.cpu.util[cpmCPUTotal5minRev.1]
				$screenItemToAdd5[] = [
					'screenid' => $screenid, 
					'resourcetype' => 1, 
					'resourceid' => $screenid * $screenid + 4, 
	...
	public function update($hosts) {
		...
		foreach ($hosts as $host) {
			...
			$hostname = $host['host'].'-screen';
			$hostid = $host['hostid'];
			DBExecute("UPDATE screens set name = '$hostname' 
				where name = (select concat(host,'-screen') from hosts where hostid = '$hostid')");
			// NGOCVB_END_CHANGE
	...
	public function delete(array $hostIds, $nopermissions = false) {
		...
		// NGOCVB_START_CHANGE
		foreach ($hostIds as $hostid) {
			DBexecute("DELETE from screens where name = (select concat(host,'-screen') from hosts where hostid = '$hostid')");
			// DBexecute("DELETE from screens_items where name = (select concat(host,'-screen') from hosts where hostid = '$hostid')");
		}
		// NGOCVB_END_CHANGE
