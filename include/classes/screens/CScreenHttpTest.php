...
class CScreenHttpTest extends CScreenBase {
	...
	public function get() {
		...
		$paging = CPagerHelper::paginate($this->page, $httptests, $sort_order,
			(new CUrl('netafier.php'))->setArgument('action', 'web.view')
		);
		...
		$table = (new CTableInfo())
			->setHeader([
				make_sorting_header(_('Host'), 'hostname', $sort_field, $sort_order, 'netafier.php?action=web.view'),
				make_sorting_header(_('Name'), 'name', $sort_field, $sort_order, 'netafier.php?action=web.view'),
				_('Number of steps'),
				_('Last check'),
				_('Status')
			]);
