...
function getMenuPopupHost(options, trigger_elmnt) {
	...
	if (options.hasGoTo) {
		...
		// latest data link
		var url = new Curl('netafier.php', false);
		...
		if (!options.showGraphs) {
			...
			var graphs_url = new Curl('netafier.php', false);
		...
		if (!options.showWeb) {
			...
			var web_url = new Curl('netafier.php', false);
...
function getMenuPopupMapElementSubmap(options) {
	...
	if (typeof options.widget_uniqueid !== 'undefined') {
		...
		submap_url = new Curl('netafier.php', false);
...
function getMenuPopupMapElementGroup(options) {
	var sections = [],
		problems_url = new Curl('netafier.php', false);
...
function getMenuPopupDashboard(options, trigger_elmnt) {
	var	url_create = new Curl('netafier.php', false),
		url_clone = new Curl('netafier.php', false),
		url_delete = new Curl('netafier.php', false);
