...
function add2favorites(object, objectid) {
	sendAjaxData('netafier.php?action=favourite.create', {
		data: {
			object: object,
			objectid: objectid
		}
...
function rm4favorites(object, objectid) {
	sendAjaxData('netafier.php?action=favourite.delete', {
		data: {
			object: object,
			objectid: objectid
		}
...
function updateUserProfile(idx, value_int, idx2) {
	return sendAjaxData('netafier.php?action=profile.update', {
		data: {
			idx: idx,
			value_int: value_int,
			idx2: idx2
